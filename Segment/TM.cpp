...
tuple<string, float>TM::segment(string input, int max_dict_word_length)
{
	int cmp = 0x3f;
	int array_size = min(max_dict_word_length,input.length());
	...
	float *log_prob_sum = new float[array_size];
	int circular_index = array_size-1;
	...
	for (int j = 0; j < input.length(); j++) {
		...
		if (j > 0) { 
			int origin = segmented_space_bits[circular_index][space_index];
			int des = (1 << ((j - 1) & cmp));
			//segmented_space_bits[circular_index][space_index] |= (1 << ((j - 1) & cmp));
			segmented_space_bits[circular_index][space_index] = origin | des;
		}

		int imax = min((input.length() - j), max_dict_word_length);
		for (int i = 1; i <= imax; i++) {
			int destination_index = ((i + circular_index) % array_size);
			string part = input.substr(j, i);
			float log_prob_part = 0;
			dict_iter = dictionary.find(part);
			if (dict_iter != dictionary.end()) {
				long float word_count = dict_iter->second;
				log_prob_part = log10(word_count / total_count);
			}
			else {
				log_prob_part = log10(10 / (total_count*pow(10, part.length())));
			}
			if(j == 0) {
